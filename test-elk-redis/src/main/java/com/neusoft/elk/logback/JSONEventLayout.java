...
public class JSONEventLayout extends LayoutBase<ILoggingEvent> {
    ...
    public synchronized String doLayout(ILoggingEvent event) {
        ...
        appendKeyNotNullValue(buf, "source", source, mdc);

        appendKeyNotNullValue(buf, "path", sourcePath, mdc);

        appendKeyNotNullValue(buf, "type", type, mdc);

        appendKeyNotNullValue(buf, "tags", tags, mdc);
    ...
    private void appendKeyNotNullValue(StringBuilder buf, String key, String value, Map<String, String> mdc) {

        if (value == null || value.length() == 0)
            return;
        value = escape(mdcSubst(value, mdc));
        if (value == null || value.length() == 0)
            return;
    ...
    private void appendKeyNotNullValue(StringBuilder buf, String key, List<String> values, Map<String, String> mdc) {
        if (values == null || values.size() == 0)
            return;
        buf.append(COMMA);
        appendKeyValue(buf, key, values, mdc);
    }
